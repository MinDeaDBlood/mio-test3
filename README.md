...
The built application keeps editable resources beside the executable:
...
`config` contains user editable settings. `languages` is scanned dynamically. `plugins/plugin_db.json` is the single local Plugin Store database. Installed plugins are stored in `plugins/installed`, while downloads and working files are stored under `temp`.

The Python package `src/platform` contains system adapters and storage code. It is not a second configuration directory. The removed packages `src/config` and `src/infrastructure` must not be restored.

The current OTA files in `templates/ota` belong to unfinished payload packing support. Active Tools windows do not currently consume them.
