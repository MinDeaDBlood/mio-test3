...
def str_to_selinux(string):
    return escape(string).replace('\\-', '-') if not string.endswith('(/.*)?') else string
...
def context_patch(fs_file, dir_path, fix_permission: dict) -> tuple:  # 接收两个字典对比
    ...
    r_new_fs = {}
    add_new = 0
    emit(f"ContextPatcher: the Original File Has {len(fs_file.keys()):d} entries")
...
def main(dir_path, fs_config, fix_permission_file) -> None:
    ...
    emit(f'ContextPatcher: Add {add_new:d} entries')
