...
class Ext4Resizer:
    ...
    def _cache_common_params(self):
        ...
        self._cached_params = {
            ...
            "inode_table_blocks": (
                sb.s_inodes_per_group * sb.s_inode_size + self.vol.block_size - 1
            )
            // self.vol.block_size,
        }
    ...
    def update_superblock(self, new_blocks_count, groups_count):
        ...
        sb = ext4_superblock.from_buffer_copy(
            self.read_data(0x400, sizeof(ext4_superblock))
        )
    ...
    def _has_metadata_csum(self):
        return (
            self.vol.superblock.s_feature_ro_compat
            & EXT4_FEATURE_RO_COMPAT_METADATA_CSUM
        ) != 0

    def _has_gdt_csum(self):
        return (
            self.vol.superblock.s_feature_ro_compat & EXT4_FEATURE_RO_COMPAT_GDT_CSUM
        ) != 0
    ...
    def update_block_group_descriptors(self, new_blocks_count, groups_count):
        ...
        self._update_group_descriptor_checksum(
            self.vol.group_descriptors[-1], groups_count - 1
        )
    ...
    def _add_new_groups(self, start_idx, end_idx, new_blocks_count):
        ...
        gdt_blocks = (end_idx * desc_size + params["block_size"] - 1) // params[
            "block_size"
        ]
        ...
        for i in range(start_idx, end_idx):
            ...
            metadata_offset = (
                (1 + gdt_blocks + params["reserved_gdt_blocks"])
                if self._group_has_super_backup(i)
                else 0
            )
    ...
    def _initialize_block_bitmap(self, group_idx, new_blocks_count):
        ...
        if self._group_has_super_backup(group_idx):
            desc_size = params["desc_size"]
            gdt_blocks = (
                len(self.vol.group_descriptors) * desc_size + block_size - 1
            ) // block_size
            ...
            used_blocks.update(
                range(
                    first_block + 1,
                    first_block + 1 + gdt_blocks + params["reserved_gdt_blocks"],
                )
            )

        inode_table_start = gd.bg_inode_table
        used_blocks.update(
            range(inode_table_start, inode_table_start + params["inode_table_blocks"])
        )
    ...
    def _group_has_super_backup(self, group_idx):
        ...
        if (
            self.vol.superblock.s_feature_ro_compat & EXT4_FEATURE_COMPAT_SPARSE_SUPER
        ) == 0:
            return True
    ...
    def physical_resize(self, new_size_bytes):
        ...
        if not os.path.isfile(self.file_path):
            emit("Cannot resize block device; use Linux resize2fs")
            return
    ...
    def resize(self, new_size):
        ...
        new_size_bytes = (
            self.parse_size(new_size) if isinstance(new_size, str) else new_size
        )
        new_blocks_count, groups_count = self.calculate_new_fs_parameters(
            new_size_bytes
        )

        current_size_bytes = (
            self.vol.get_block_count * self._cached_params["block_size"]
        )
        used_blocks = self.vol.get_block_count - self.vol.get_free_blocks_count
        ...
        if new_blocks_count < min_required_blocks:
            raise ResizeError(
                f"Minimum {min_required_blocks} blocks required for current data"
            )

        self.check_resize_constraints(new_blocks_count)

        emit(
            f"Size:   {_human_size(current_size_bytes)} -> {_human_size(new_size_bytes)}"
        )
        emit(f"Blocks: {self.vol.get_block_count} -> {new_blocks_count}")
        emit(f"Used:   {used_blocks}")
        emit(
            f"Free:   {self.vol.get_free_blocks_count} -> {new_blocks_count - used_blocks}"
        )
...
def main(file_path, new_size):
    ...
    try:
        with Ext4Resizer(file_path) as resizer:
            if not new_size:
                used_blocks = (
                    resizer.vol.get_block_count - resizer.vol.get_free_blocks_count
                )
                buffer_blocks = max(
                    int(used_blocks * 0.01),
                    int(1024 * 1024 / resizer._cached_params["block_size"]),
                )
                new_size_bytes = (used_blocks + buffer_blocks) * resizer._cached_params[
                    "block_size"
                ]
                emit(
                    f"Auto-size: adding {buffer_blocks} block buffer (1% or 1MB minimum)"
                )
