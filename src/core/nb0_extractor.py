...
def write_dword(fd, value):
    fd.write(chr(value & 0xFF).encode())
    fd.write(chr((value >> 8) & 0xFF).encode())
    fd.write(chr((value >> 16) & 0xFF).encode())
    fd.write(chr((value >> 24) & 0xFF).encode())
...
def extract_nb0(file_name: str, extract_to: str) -> int:
    ...
    with open(file_name, "rb") as f:
        ...
        size = read_dword(f)
        if size > (fsize / 64):
            emit("ERROR invalid nb0 file")
            return 1
        emit(f"File count: {size}")
        lastoffset = 0
        i = 0
        while i < size:
            nb = nb0_header_item()
            ...
            if not len(f.read(1)) or nb.offset < lastoffset:
                emit("ERROR invalid nb0 file")
                return -1
            emit(f"offset = {nb.offset:08x} size = {nb.size:08x} name: '{nb.name}'\n")
            i += 1
        f.seek(64 * size + 4, SEEK_SET)
        emit(f"fileops {f.tell()}")
        i = 0
        while i < size:
            nb = nb0_headers[i]
            ...
            if extract_to:
                ...
                emit(f"extracting to {buf} ({nb.size} bytes)\n")
                ...
                while sz > 0:
                    f.seek(f.tell() - 1)
                    if not len(f.read(1)):
                        emit("ERROR unexpected end of file")
                        return 1
