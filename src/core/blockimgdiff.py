...
def _format_message(
    _message_code: str, default_text: str | None = None, **values
) -> str:
    text = default_text or _message_code
...
class DataImage(Image):
    ...
    def __init__(self, data: bytes, trim=False, pad=False):
        self.data = data
        ...
        if (
            self.clobbered_blocks.size() > 0
        ):  # If there are clobbered blocks (i.e., it was padded)
...
class Transfer:
    ...
    def __str__(self):
        return (
            f"{self.id}: <{self.src_ranges} {self.style} to {self.tgt_ranges}> "
...
class HeapItem:
    ...
    def __lt__(
        self, other
    ):  # Changed from __le__ to __lt__ for standard total_ordering
        if not isinstance(other, HeapItem):
            return NotImplemented
        return self.score < other.score
...
class BlockImageDiff:
    def __init__(
        ...
        self.tool_path_resolver = tool_path_resolver or (
            lambda name: name
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                error_msg = _format_message(
                    ...
                    tool=tool_name,
                    target_name=xf.tgt_name,
                    source_name=xf.src_name,
    ...
    def WriteTransfers(self, prefix: str):
        ...
        for xf in self.transfers:
            ...
            for stash_id_or_sha, stash_ranges in xf.stash_before:
                if self.version == 2:  # stash_id_or_sha is an ID
                    ...
                    if sha_hex not in stashes:
                        ...
                        out_lines.append(
                            f"stash {sha_hex} {stash_ranges.to_string_raw()}\n"
                        )
                    stashes[sha_hex] += 1
                if (
                    self.version != 2
                ):  # For v3+, stashed_blocks_count updated above for unique SHAs
                    pass  # Stashed_blocks_count for v3+ is based on unique content
                else:  # For v2, all stashed blocks count
                    stashed_blocks_count += stash_ranges.size()

            max_stashed_blocks_count = max(
                max_stashed_blocks_count, stashed_blocks_count
            )
            ...
            if self.version >= 2:
                ...
                for stash_id_or_sha, ranges_to_use_from_stash in xf.use_stash:
                    ...
                    unstashed_src_ranges_in_xf = unstashed_src_ranges_in_xf.subtract(
                        ranges_to_use_from_stash
                    )

                    if self.version == 2:
                        sid = stash_id_or_sha
                        assert stashes.pop(sid, None), (
                            f"Stash ID {sid} not found for use"
                        )
                        src_str_parts.append(
                            f"{sid:d}:{sr_in_xf_space.to_string_raw()}"
                        )
                        heappush(free_stash_ids_v2, sid)  # Make ID available again
                        temp_free_cmds.append(f"free {sid:d}\n")
                        temp_freed_blocks_count += (
                            ranges_to_use_from_stash.size()
                        )  # Original size
                    else:  # self.version >= 3
                        sha_hex = stash_id_or_sha
                        assert sha_hex in stashes and stashes[sha_hex] > 0, (
                            f"Stash SHA {sha_hex} not found or count is zero"
                        )
                        stashes[sha_hex] -= 1
                        src_str_parts.append(
                            f"{sha_hex}:{sr_in_xf_space.to_string_raw()}"
                        )
                ...
                if unstashed_src_ranges_in_xf.size() > 0:
                    ...
                    if (
                        xf.use_stash
                    ):  # If stashes were used, need to add mapped unstashed ranges
                        mapped_unstashed = xf.src_ranges.map_within(
                            unstashed_src_ranges_in_xf
                        )
                    ...
                    if mapped_stashes_for_xf:
                        self.AssertPartition(
                            RangeSet(data=(0, src_size)), mapped_stashes_for_xf
                        )
            ...
            if xf.style == "new":
                ...
                assert (
                    xf.tgt_ranges.size() > 0
                    and xf.src_ranges.size() == tgt_size_current_xf
                )
                if (
                    xf.src_ranges != xf.tgt_ranges
                ):  # No command if src and tgt ranges are identical
                    if self.version == 1:
                        cmd_line = f"move {xf.src_ranges.to_string_raw()} {xf.tgt_ranges.to_string_raw()}\n"
                    elif self.version == 2:
                        cmd_line = (
                            f"move {xf.tgt_ranges.to_string_raw()} {final_src_str}\n"
                        )
                    else:  # version >= 3
                        if xf.src_ranges.overlaps(
                            xf.tgt_ranges
                        ):  # Potential implicit stash
                            max_stashed_blocks_count = max(
                                max_stashed_blocks_count,
                                stashed_blocks_count + xf.src_ranges.size(),
                            )
                        self.touched_src_ranges = self.touched_src_ranges.union(
                            xf.src_ranges
                        )
                ...
                ranges_to_make_zero = (
                    xf.tgt_ranges
                )  # Original interpretation implies all of tgt_ranges becomes zero.
                ...
                written_count = self._write_zero_commands(
                    out_lines, ranges_to_make_zero
                )  # Appends to out_lines
                tgt_size_current_xf = written_count  # Actual blocks zeroed
                cmd_line = None  # Commands already added by _write_zero_commands
            else:
                raise ValueError(
                    _format_message(
                        "imgdiff_error_unknown_transfer_style",
                        default_text="Unknown transfer style '{style}'",
                        style=xf.style,
                    )
                )
            ...
            if self.version >= 2 and self.cache_size_bytes is not None:
                ...
                current_max_stashed_bytes = (
                    max_stashed_blocks_count * self.tgt.blocksize
                )
                if current_max_stashed_bytes > max_allowed_stash_bytes:
                    error_msg = _format_message(
                        ...
                        stash_bytes=current_max_stashed_bytes,
                        stash_blocks=max_stashed_blocks_count,
                        block_size=self.tgt.blocksize,
                        max_allowed_bytes=int(max_allowed_stash_bytes),
                        cache_size_bytes=self.cache_size_bytes,
                        threshold=self.stash_threshold,
                    )
        ...
        target_blocks_minus_extended = all_target_device_blocks.subtract(
            self.tgt.extended
        )
        new_image_dont_care_blocks = target_blocks_minus_extended.subtract(
            self.tgt.care_map
        )

        erase_at_beginning = new_image_dont_care_blocks.subtract(
            ...
            if self.version == 2:  # For v2, count actual unique stash IDs used.
                ...
                for xf_check_stash in self.transfers:
                    for sid, _ in xf_check_stash.stash_before:
                        if self.version == 2:
                            max_sid_in_use_v2 = max(max_sid_in_use_v2, sid)
                actual_next_stash_id_v2 = (
                    max_sid_in_use_v2 + 1 if max_sid_in_use_v2 != -1 else 0
                )
        ...
        if self.cache_size_bytes is not None:
            ...
            percentage = (
                (self._max_stashed_size * 100.0 / max_allowed_b)
                if max_allowed_b > 0
                else float("inf")
            )
            self.logger.info(
                _format_message(
                    "imgdiff_info_max_stashed_limit",
                    default_text="Max stashed blocks: {blocks} ({bytes} bytes), limit: {limit_bytes} bytes ({percentage:.2f}%)",
                    blocks=max_stashed_blocks_count,
                    bytes=self._max_stashed_size,
                    limit_bytes=int(max_allowed_b),
            ...
            self.logger.info(
                _format_message(
                    "imgdiff_info_max_stashed_unknown_limit",
    ...
    def _write_zero_commands(
        self, out_lines_list, ranges_to_zero: RangeSet, limit_per_cmd=1024
    ):
    ...
    def ReviseStashSize(self):
        ...
        self.logger.info(
            _format_message(
                "imgdiff_info_revising_stash", default_text="Revising stash size..."
            )
        )
        ...
        for xf in self.transfers:
            ...
            for id_or_sha, _ in xf.use_stash:
                ...
                if (
                    id_or_sha in stash_definitions
                ):  # Should always be true if graph is correct
                    ...
                    self.logger.warning(
                        f"Stash {id_or_sha} used by {xf} but no definition found."
                    )

        max_allowed_blocks = (
            self.cache_size_bytes * self.stash_threshold
        ) // self.tgt.blocksize
        ...
        for xf in self.transfers:
            ...
            for id_or_sha, sr_stashed in xf.stash_before:
                ...
                if current_stashed_blocks + sr_stashed.size() > max_allowed_blocks:
                    ...
                    if use_cmd and use_cmd.style != "new":  # Avoid re-converting
                        ...
                        self.logger.info(
                            f"  Will convert {use_cmd} (uses {id_or_sha}) to NEW due to stash limit for explicit stash {sr_stashed.size()} blocks."
                        )
                else:
            ...
            for cmd_to_new in commands_to_convert_to_new:
                ...
                for used_id_or_sha, used_sr in cmd_to_new.use_stash:
                    _, def_cmd, _ = stash_definitions.get(
                        used_id_or_sha, (None, None, None)
                    )
                    if def_cmd:
                        try:
    ...
    def ComputePatches(self, prefix: str):
        ...
        self.logger.info(
            _format_message(
                "imgdiff_info_preparing_diff_data",
                default_text="Preparing data for diffing...",
            )
        )
        ...
        for xf in self.transfers:
            if xf.style == "new":
                ...
                xf.style == "diff"
            ):  # Original style before checking for identical data
                ...
                if src_sha1.digest() == tgt_sha1.digest():
                    ...
                    use_imgdiff = (
                        not self.disable_imgdiff
                        and xf.intact
                        and xf.tgt_name.split(".")[-1].lower() in ("apk", "jar", "zip")
                    )
                    ...
                    diff_tasks.append(
                        (src_data_list, tgt_data_list, xf, tgt_byte_count)
                    )
        ...
        computed_patches = [None] * len(
            diff_tasks
        )  # To store (patch_data, original_xf_object)

        if diff_tasks:
            if self.threads > 1:
                self.logger.info(
                    _format_message(
                        "imgdiff_info_computing_patches_threads",
                ...
                self.logger.info(
                    _format_message(
                        "imgdiff_info_computing_patches_single_thread",
            ...
            )  # item[3] is tgt_byte_count

            with ThreadPoolExecutor(max_workers=self.threads) as executor:
                future_to_xf_idx = {
                    executor.submit(
                        self._compute_patch_for_transfer, src_d, tgt_d, xf_task
                    ): idx
                ...
                for future in as_completed(future_to_xf_idx):
                    ...
                    try:
                        ...
                        percentage = (
                            (patch_size_bytes * 100.0 / tgt_size_bytes)
                            if tgt_size_bytes > 0
                            else 0.0
                        )
                        ...
                        self.logger.info(
                            _format_message(
                                "imgdiff_info_patch_stats",
                                default_text="{patch_size:>10d} {target_size:>10d} ({percentage:6.2f}%) {style:>7} {name}",
                                patch_size=patch_size_bytes,
                                target_size=tgt_size_bytes,
                                percentage=percentage,
                                style=xf_task.style,
                                name=log_name,
                            )
                        ...
                        self.logger.error(
                            _format_message(
                                "imgdiff_error_patch_computation_failed",
                                default_text="Patch computation failed for {target_name}: {error}",
                                target_name=xf_task.tgt_name,
                                error=str(e),
                            )
                        )
            ...
            for result, xf_task_check in computed_patches:
                if isinstance(result, Exception):
                    ...
                    raise RuntimeError(
                        f"Patch computation failed for {xf_task_check.tgt_name}. See logs for details."
                    ) from result
        ...
        with open(patch_dat_path, "wb") as f_patch:
            ...
            for item in computed_patches:
                if item is None:
                    continue  # Should not happen if all tasks processed
                patch_data, xf_ref = item

                if isinstance(
                    patch_data, Exception
                ):  # Already handled by raising above
                    continue
        ...
        for xf in self.transfers:
            ...
            if self.version >= 2:
                for _, stashed_ranges_used_by_xf in xf.use_stash:
                    source_blocks_from_device = source_blocks_from_device.subtract(
                        stashed_ranges_used_by_xf
                    )
    ...
    def ImproveVertexSequence(self):
        self.logger.info(
            _format_message(
                "imgdiff_info_improving_order", default_text="Improving vertex order..."
            )
        )
        for xf in self.transfers:
        ...
        source_nodes = [
            (u.NetStashChange(), u.order, u) for u in self.transfers if not u.incoming
        ]
        ...
        while source_nodes:
            ...
            for neighbor_xf in list(
                current_xf.outgoing.keys()
            ):  # Iterate copy as it's modified
                del neighbor_xf.incoming[
                    current_xf
                ]  # Remove edge current_xf -> neighbor_xf
                if not neighbor_xf.incoming:  # If neighbor_xf becomes a source node
                    heappush(
                        source_nodes,
                        (neighbor_xf.NetStashChange(), neighbor_xf.order, neighbor_xf),
                    )

        assert len(ordered_transfers) == len(self.transfers), (
            "Cycle detected or error in topological sort."
        )
    ...
    def RemoveBackwardEdges(self):  # Specific to version 1
        self.logger.info(
            _format_message(
                "imgdiff_info_removing_backward_edges",
        ...
        for xf in self.transfers:
            ...
            for u_neighbor in list(xf.goes_before.keys()):  # Iterate copy
                if xf.order < u_neighbor.order:  # Edge is in correct order
                    ...
                    xf.intact = (
                        False  # Source integrity potentially compromised for imgdiff
                    )
        ...
        if (in_order_deps + out_of_order_deps) > 0:
            percentage_violated = (
                out_of_order_deps * 100.0 / (in_order_deps + out_of_order_deps)
            )
        else:
        ...
        self.logger.info(
            _format_message(
                "imgdiff_info_backward_edges_stats_v1",
                default_text="  {out_of_order}/{total} dependencies ({percentage:.2f}%) were violated; {lost_source} source blocks removed.",
                out_of_order=out_of_order_deps,
                total=(in_order_deps + out_of_order_deps),
                percentage=percentage_violated,
                lost_source=total_lost_source_blocks,
            )
        )
    ...
    def ReverseBackwardEdges(self):  # Specific to version >= 2
        self.logger.info(
            _format_message(
                "imgdiff_info_reversing_backward_edges",
                default_text="Reversing backward edges (v2+ method)...",
            )
        )
        in_order_deps = 0
        out_of_order_deps = 0
        next_stash_entity_id = (
            0  # Unique ID for v2 stashes, or placeholder for v3+ SHA logic
        )
        ...
        for xf in self.transfers:
            ...
            for u_neighbor in list(xf.goes_before.keys()):  # Iterate copy
                if xf.order < u_neighbor.order:  # Edge is in correct order
                    ...
                    u_neighbor.goes_before[xf] = (
                        None  # Weight/value might not be critical after reversal
                    )
    ...
    def FindVertexSequence(self):
        self.logger.info(
            _format_message(
                "imgdiff_info_finding_vertex_sequence",
        ...
        s2_sequence = (
            deque()
        )  # Sequence from right to left (elements added to left of deque)
        ...
        def update_score_in_heap(item_to_update, delta_score):
            ...
            item_to_update.heap_item = HeapItem(
                item_to_update
            )  # New HeapItem with updated score
        ...
        while G_nodes:
            ...
            while sinks_in_G:
                ...
                if u_sink not in G_nodes:
                    continue  # Already processed

                s2_sequence.appendleft(u_sink)
                del G_nodes[u_sink]
                if hasattr(u_sink, "heap_item") and u_sink.heap_item:
                    u_sink.heap_item.clear()  # Remove from heap consideration
                ...
                for v_predecessor in list(u_sink.incoming.keys()):
                    if v_predecessor in G_nodes:  # If predecessor is still in G
                        ...
                        update_score_in_heap(
                            v_predecessor, -edge_weight
                        )  # Score increases as an outgoing edge is removed
                        if (
                            not v_predecessor.outgoing
                        ):  # If v_predecessor becomes a sink
            ...
            while sources_in_G:
                ...
                if u_source not in G_nodes:
                    continue

                s1_sequence.append(u_source)
                del G_nodes[u_source]
                if hasattr(u_source, "heap_item") and u_source.heap_item:
                    u_source.heap_item.clear()
                ...
                for v_successor in list(u_source.outgoing.keys()):
                    if v_successor in G_nodes:
                        ...
                        update_score_in_heap(
                            v_successor, +edge_weight
                        )  # Score decreases as an incoming edge is removed
            ...
            while active_heap:
                ...
                if (
                    heap_item_u and heap_item_u.item in G_nodes
                ):  # Check if item is valid and in G
                    selected_u = heap_item_u.item
                    break

            if not selected_u:  # Should not happen if G_nodes is not empty
                if G_nodes:
                    raise AssertionError("Heap empty but G_nodes not empty")
                break
            ...
            for v_successor in list(selected_u.outgoing.keys()):
                if v_successor in G_nodes:
                    ...
                    if not v_successor.incoming:
                        sources_in_G.add(v_successor)  # Became a source
            ...
            for v_predecessor in list(selected_u.incoming.keys()):
                if v_predecessor in G_nodes:
                    ...
                    if not v_predecessor.outgoing:
                        sinks_in_G.add(v_predecessor)  # Became a sink

        # Combine s1 and s2 to form the final sequence
        final_ordered_transfers = list(s1_sequence) + list(s2_sequence)
        assert len(final_ordered_transfers) == len(self.transfers), (
        ...
        for i, xf in enumerate(self.transfers):
            ...
            del xf.outgoing
            if hasattr(xf, "heap_item"):
                del xf.heap_item

    def GenerateDigraph(self):
        self.logger.info(
            _format_message(
                "imgdiff_info_generating_digraph", default_text="Generating digraph..."
            )
        )
        ...
        for (
            xf_b
        ) in self.transfers:  # 'b' for the transfer that reads (is a source user)
        ...
        for xf_a in self.transfers:
            ...
            intersecting_readers = (
                set()
            )  # Set of xf_b's that read blocks xf_a writes to.
            for s, e in xf_a.tgt_ranges:  # Blocks written by xf_a
                for block_idx in range(s, e):
                    if (
                        block_idx < max_src_block_idx
                        and source_block_readers[block_idx] is not None
                    ):
                        intersecting_readers.update(source_block_readers[block_idx])

            for xf_b in intersecting_readers:
                if xf_a is xf_b:
                    continue  # A transfer cannot depend on itself in this manner
                ...
                if overlap_ranges.size() > 0:
                    ...
                    edge_weight = (
                        0 if xf_b.src_name == "__ZERO" else overlap_ranges.size()
                    )
    ...
    def FindTransfers(self):
        ...
        if (
            self.version >= 3
            and self.cache_size_bytes is not None
            and self.tgt.blocksize > 0
        ):
            max_blocks_per_diff_piece = int(
                (self.cache_size_bytes * 0.125) / self.tgt.blocksize
            )
        ...
        for tgt_fn, tgt_fn_ranges in self.tgt.file_map.items():
            if tgt_fn == "__ZERO":
                ...
                self._AddTransferWrapper(
                    tgt_fn,
                    "__ZERO",
                    tgt_fn_ranges,
                    src_fn_ranges,
                    "zero",
                    max_blocks_per_diff_piece,
                )
            elif (
                tgt_fn == "__COPY"
            ):  # Unconditional copy from target image data (effectively "new")
                self._AddTransferWrapper(
                    tgt_fn,
                    None,
                    tgt_fn_ranges,
                ...
                if tgt_fn in self.src.file_map:  # Exact path match
                    ...
                    if basename_tgt in self.src_basenames:  # Exact basename match
                        ...
                        if (
                            num_pattern_tgt in self.src_numpatterns
    ...
    def _AddTransferWrapper(
        ...
        if (
            style == "diff"
            and self.version >= 3
            and split_threshold_blocks > 0
            and (
                tgt_ranges.size() > split_threshold_blocks
                or src_ranges.size() > split_threshold_blocks
            )
        ):
            piece_num = 0
            ...
            while (
                current_tgt_ranges.size() > split_threshold_blocks
                and current_src_ranges.size() > split_threshold_blocks
            ):
                tgt_piece = current_tgt_ranges.first(split_threshold_blocks)
                src_piece = current_src_ranges.first(split_threshold_blocks)

                Transfer(
                    f"{tgt_name}-p{piece_num}",
                    f"{src_name}-p{piece_num}" if src_name else None,
                    tgt_piece,
                    src_piece,
                    style,
                    self.transfers,
            ...
            if current_tgt_ranges.size() > 0 or current_src_ranges.size() > 0:
                if (
                    style == "diff"
                ):  # For diff, both must have remaining parts or it's an issue
                    assert (
                        current_tgt_ranges.size() > 0 and current_src_ranges.size() > 0
    ...
    def AbbreviateSourceNames(self):
        ...
        for k_path in self.src.file_map.keys():
            if k_path.startswith("__"):
                continue  # Skip special domains

            b_name = os.path.basename(k_path)
            if (
                b_name not in self.src_basenames
            ):  # Prioritize first encountered if multiple paths have same basename
