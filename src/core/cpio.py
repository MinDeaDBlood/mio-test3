    ...
    with open(filename, 'rb') as f:
        while True:
            ...
            try:
                ...
                emit("\n--- DECODING ERROR ---")
                emit("Failed to decode a filename as UTF-8.")
                emit(f"Problematic bytes in HEX format: {hex_representation}")
                emit("Use an online converter to determine the correct encoding.")
                emit("----------------------\n")
            ...
            if (header.c_magic == CpioMagicFormat.Crc.value) and check_crc:
                emit(f"CRC State:{calc_crc(file_content) == int(header.c_chksum.decode('utf-8'), 16)}")
            if file_type == CpioModes.C_ISREG:
                ...
                emit(f"Unsupported Type:{file_type}")
            emit(f"Extracted:{name}")
            if file_size % 4:
...
def repack(input_dir, config_file, output_file: str, magic_type: CpioMagicFormat = None):
    ...
    with open(output_file, 'wb') as out:
        ...
        for entry in scan_dir(input_dir):
            ...
            if ino_ran == ino_sum:
                emit(f"Warning: {entry}:{ino_ran} == {ino_sum}")
                ino_ran += 1
            is_file = False
            is_link = False
            emit(f'adding: {entry}')
