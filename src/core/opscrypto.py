...
class QCSparse:
    ...
    def readheader(self, offset):
        ...
        if self.file_hdr_sz != 28:
            self.error(
                f"The file header size was expected to be 28, but is {self.file_hdr_sz:d}."
            )
            return False
        if self.chunk_hdr_sz != 12:
            emit(
                f"The chunk header size was expected to be 12, but is {self.chunk_hdr_sz:d}."
            )
            return False
        emit("Sparse Format detected. Using unpacked image.")
        return True
    ...
    def get_chunk_size(self):
        ...
        if chunk_type == 0xCAC1:
            ...
            if data_sz != 4:
                self.error(
                    f"Fill chunk should have 4 bytes of fill, but this has {data_sz:d}"
                )
                return -1
            else:
            ...
            if data_sz != 4:
                self.error(
                    f"CRC32 chunk should have 4 bytes of CRC, but this has {data_sz:d}"
                )
                return -1
            else:
            ...
            emit(f"Unknown chunk type 0x{chunk_type:04X}")
            return -1
...
def extractxml(filename, key):
    with mmap_io(filename, "rb") as rf:
        sfilename = os.path.join(
            filename[: -len(os.path.basename(filename))], "extract", "settings.xml"
        )
...
def main(args):
    ...
    if args["decrypt"]:
        ...
        emit("Done. Extracted files to " + path)
            ...
            try:
                ...
                with open(outfilename, "rb") as rt:
                    with open("md5sum_pack.md5", "wb") as wt:
                        ...
                        mt.update(rt.read())
                        wt.write(
                            bytes(mt.hexdigest(), "utf-8")
                            + b" "
                            + bytes(os.path.basename(outfilename), "utf-8")
                        )
                emit("Done. Created " + outfilename)
