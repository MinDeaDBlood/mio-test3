...
def brutekey(rf):
    ...
    raise CoreOperationError(
        "Unknown OFP encryption key. This firmware is not supported",
        exit_code=1,
    )


def cleancstring(string):
    return string.replace(b"\x00", b"").decode("utf-8")
...
def main(filename, outdir):
    ...
    with open(filename, "rb") as rf:
        ...
        prjname, _, _, cpu, flashtype, hdr2entries, prjinfo, _ = unpack(
            "46s Q 4s 7s 5s H 32s H", hdr
        )
        ...
        if prjname:
            emit(f"Detected prjname:{prjname}")
        if prjinfo:
            emit(f"Detected prjinfo:{prjinfo}")
        if cpu:
            emit(f"Detected cpu:{cpu}")
        if flashtype:
            emit(f"Detected flash:{flashtype}")
        ...
        for i in range(0, len(hdr2) // 0x60):
            # _=crc
            name, start, length, enclength, filename, _ = unpack(
            ...
            emit(f'Writing "{name}" as "{outdir}/{filename}"...')
    ...
    emit(f"Files successfully decrypted to subdirectory {outdir}")
