    ...
    def keytest(data):
        for key in keys:
            if AES.new(binascii.unhexlify(key), AES.MODE_ECB).decrypt(data)[0:4] in [b'\x50\x4B\x03\x04',
                                                                                     b'\x41\x56\x42\x30',
                                                                                     b'\x41\x4E\x44\x52']:
                emit(f"Found correct AES key: {key}")
                        ...
                        if "boot.img" in orgfilename:
                            ...
                            with open(os.path.join(temp, "out"), 'rb') as rr:
                                if rr.read(12) == b"OPPOENCRYPT!":
                                    ...
                                    if key == -1:
                                        emit("Unknown AES key, reverse key from recovery first!")
                                        return 1
                                    else:
                                        break
                                else:
                                    emit("Unknown mode2, boot.img wasn't encrypted")
                                    break

                    emit(f"Extracting...  {file_arg}")
                        ...
                        for zi in zipObj.infolist():
                            ...
                            with open(os.path.join(temp, "out"), 'rb') as rr:
                                if rr.read(12) == b"OPPOENCRYPT!":
                                    ...
                                    os.remove(os.path.join(temp, "out"))
                    rmrf(temp)
                    emit(f"DONE... file decrypted to: {outzip}")
                    return 0
            else:
                return 1

    emit("ozipdecrypt 1.32 (c) B.Kerler 2017-2022")
    filename = file_arg
    with open(filename, 'rb') as fr:
        magic = fr.read(12)
        if magic == b"OPPOENCRYPT!":
            ...
            with zipfile.ZipFile(filename, 'r') as zo:
                    ...
                    for n in ["firmware-update/vbmeta.img", "vbmeta.img"]:
                        if n in clist:
                            if zo.extract(n, outpath):
                                ...
                                emit("Unknown image, please report an issue with image name!")
                                return 1

                outzip: str = filename[:-4] + 'zip'
                with zipfile.ZipFile(outzip, 'w', zipfile.ZIP_DEFLATED) as WzipObj:
                    for info in zo.infolist():
                        emit(f"Extracting {info.filename}")
                        ...
                        if len(clist) > 0:
                            if info.filename in clist:
                                emit(f"Decrypting {info.filename}")
                ...
                emit(f"DONE... files decrypted to: {outzip}")
                return 0
        else:
            emit(f"ozip has unknown magic {magic}, OPPOENCRYPT! expected!")
            return 1
