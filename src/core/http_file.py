...
class HttpFile(RawIOBase):
    ...
    def readinto(self, buffer) -> int:
        # emit(f'read into from {self.pos}-{end_pos}')
        return self._read_internal(buffer)

    def seek(self, offset: int, whence: int = os.SEEK_SET) -> int:
        # emit(f'seek to {offset} whence {whence}')
        ...
        # emit(f'seek: pos {self.pos} -> {new_pos}')
        self.pos = new_pos
        return new_pos
...
if __name__ == "__main__":
    ...
    with HttpFile(
        ...
        emit("file size:", f.tell())
        f.seek(0, os.SEEK_SET)
        z = zipfile.ZipFile(f)
        emit(z.namelist())
        for name in z.namelist():
            with z.open(name) as payload:
                emit(name, "compress type:", payload._compress_type)
        emit("total read:", f.total_bytes)
        ...
        for name in z.namelist():
            with z.open(name) as payload:
                emit(
                    name,
                    "compress type:",
                    payload._compress_type,
                    "size:",
                    payload._left,
                )
        with z.open(
            "comet_beta-bp21.241121.009/image-comet_beta-bp21.241121.009.zip"
        ) as f2:
