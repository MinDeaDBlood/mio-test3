...
class KDZFileTools(kdz.KDZFile):
    ...
    def loadParams(self):
        ...
        while len(line) > 0:
            ...
            if len(parts[1]) == 0:
                emit("[!] Bad line in {:s}'s parameter file".format(self.indir), file=sys.stderr)
            var = parts[0].rstrip()
            try:
        ...
        for k in 'version', 'dataStart', 'payload0', 'payload0head':
            if k not in params:
                emit("Parameter value \"{:s}\" is missing, unable to continue".format(k))
                raise CoreOperationError(f'Missing required KDZ parameter: {k}', exit_code=1)

        if params['version'] != 2:
            emit("[!] File format version not 2, cannot continue", file=sys.stderr)
            raise CoreOperationError('Unsupported KDZ file format version', exit_code=1)
    ...
    def cmdCreateFile(self):
        ...
        for name in self.payload:
            emit("[+] Writing {:s} to output file {:s}".format(name, self.kdzfile))
