...
class Extractor:
    ...
    @staticmethod
    def __write(msg, log):
        directory = os.path.dirname(os.path.abspath(log))
        if directory:
            os.makedirs(directory, exist_ok=True)
        with open(log, 'w', newline='\n', encoding='utf-8') as file:
            file.write(str(msg).strip())
            file.write('\n')
        ...
        for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
            ...
            if entry_inode.is_dir:
                if os.name == 'nt' and ":" in entry_inode_path:
                    emit("[NTWarning] The <:> not allow in path, will replace <:> to <_>.")
                ...
                try:
                    ...
                    emit(f'[E] Cannot Write to {file_target}, Reason: {e}')
                if os.name == 'posix' and os.geteuid() == 0:
    ...
    def main(self, target: str, output_dir: str, work: str, target_type: str = 'img'):
        ...
        if target_type == 'img':
            ...
            if re.search(b'\x4d\x4f\x54\x4f', data):
                emit(".....MOTO structure! Fixing.....")
                self.fix_moto(os.path.abspath(self.OUTPUT_IMAGE_FILE))
            self.fix_size()
            emit(f"Extracting {os.path.basename(target)} --> {os.path.basename(self.EXTRACT_DIR)}")
            start = dti()
