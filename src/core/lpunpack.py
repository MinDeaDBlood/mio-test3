...
class Metadata:
    ...
    def _get_info(self) -> Dict:
        ...
        try:
            ...
            emit("[Lpunpack] Cannot get info of this super image")
        return result
...
class SparseImage:
    _STREAM_READ_SIZE = 16 * 1024 * 1024
    ...
    def _read_data(self, chunk_data_size: int):
        ...
        while remaining > 0:
            request_size = min(remaining, self._STREAM_READ_SIZE)
            while True:
                try:
                    block = self._fd.read(request_size)
                    break
                except OverflowError:
                    if request_size <= 1:
                        raise
                    request_size = max(1, request_size // 2)
            if not block:
                raise LpUnpackError('Unexpected EOF while reading sparse chunk data.')
            remaining -= len(block)
            yield block

    def _consume_data(self, chunk_data_size: int):
        for _ in self._read_data(chunk_data_size):
            pass

    def _write_fill_chunk(self, out: IO[bytes], fill_value: bytes, expanded_size: int):
        if len(fill_value) != 4:
            raise LpUnpackError('Fill chunk should contain exactly 4 bytes of pattern data.')

        pattern_repeats = max(1, self._STREAM_READ_SIZE // len(fill_value))
        pattern_block = fill_value * pattern_repeats
        remaining = expanded_size
        while remaining > 0:
            current_size = min(remaining, len(pattern_block))
            out.write(pattern_block[:current_size])
    ...
    def unsparse(self):
        ...
        with open(str(unsparse_file), 'wb') as out:
            while chunks > 0:
                chunk_header_data = self._fd.read(SPARSE_CHUNK_HEADER_SIZE)
                if len(chunk_header_data) != SPARSE_CHUNK_HEADER_SIZE:
                    raise LpUnpackError('Unexpected EOF while reading sparse chunk header.')

                chunk_header = SparseChunkHeader(chunk_header_data)
                expanded_size = chunk_header.chunk_sz * self.header.blk_sz
                chunk_data_size = chunk_header.total_sz - self.header.chunk_hdr_sz
                if chunk_data_size < 0:
                    raise LpUnpackError('Sparse chunk payload size is invalid.')

                if chunk_header.chunk_type == 0xCAC1:
                    if chunk_data_size != expanded_size:
                        raise LpUnpackError(
                            f'Raw chunk input size ({chunk_data_size}) does not match output size ({expanded_size}).'
                        )
                    for block in self._read_data(chunk_data_size):
                        out.write(block)
                elif chunk_header.chunk_type == 0xCAC2:
                    if chunk_data_size != 4:
                        raise LpUnpackError(
                            f'Fill chunk should contain 4 bytes of pattern data, got {chunk_data_size}.'
                        )
                    fill_value = b''.join(self._read_data(chunk_data_size))
                    self._write_fill_chunk(out, fill_value, expanded_size)
                elif chunk_header.chunk_type == 0xCAC3:
                    self._consume_data(chunk_data_size)
                    out.seek(expanded_size, os.SEEK_CUR)
                    out.truncate(out.tell())
                elif chunk_header.chunk_type == 0xCAC4:
                    if chunk_data_size != 4:
                        raise LpUnpackError(
                            f'CRC32 chunk should contain 4 bytes of checksum data, got {chunk_data_size}.'
                        )
                    self._consume_data(chunk_data_size)
                else:
                    raise LpUnpackError(f'Unknown sparse chunk type: 0x{chunk_header.chunk_type:04X}')
                chunks -= 1
...
class LpUnpack:
    ...
    def _extract_partition(self, unpack_job: UnpackJob):
        ...
        start = dti()
        emit(f'Extracting partition [{unpack_job.name}]')
        ...
        for index, offset in enumerate(offsets):
            ...
            if header.magic != LP_METADATA_HEADER_MAGIC:
                ...
                if check_index >= len(offsets):
                    ...
                    emit(f'Read Backup header by offset 0x{offsets[check_index]:x}')
                    continue
    ...
    def get_parts(self):
        try:
            if SparseImage(self._fd).check():
                emit('Sparse image detected.')
                emit('Process conversion to non sparse image...')
    ...
    def unpack(self):
        try:
            ...
            raise CoreOperationError(e.message, exit_code=1) from e
