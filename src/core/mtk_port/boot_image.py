...
import struct
import hashlib
from stat import S_IFDIR, S_IFLNK, S_IFREG, S_IMODE, S_ISDIR, S_ISLNK, S_ISREG
import shutil
...
def write_bootimg(
    ...
    if not isinstance(base, int):
        ...
        _log("base is %s, using default base instead.\n" % type(base))

    if not isinstance(ramdisk_addr, int):
        ramdisk_addr = base + 0x01000000
        _log(
            "ramdisk_addr is %s, using default ramdisk_addr instead.\n"
            % type(ramdisk_addr)
        )
    ...
    if not isinstance(second_addr, int):
        ...
        _log(
            "second_addr is %s, using default second_addr instead.\n"
            % type(second_addr)
        )

    if not isinstance(tags_addr, int):
        tags_addr = base + 0x00000100
        _log("tags_addr is %s, using default tags_addr instead.\n" % type(tags_addr))

    if not isinstance(page_size, int):
        page_size = 0x800
        _log("page_size is %s, using default page_size instead.\n" % type(page_size))

    if not isinstance(padding_size, int):
        padding_size = 0x800  # 0x1000?
        _log(
            "padding_size is %s, using default padding_size instead.\n"
            % type(padding_size)
        )

    if not hasattr(output, "write"):
        raise TypeError("output must be a writable binary file object")

    def padding(x):
        return struct.pack("%ds" % ((~x + 1) & (padding_size - 1)), b"")
...
def parse_bootimg(bootimg):
    ...
    if not base == ramdisk_addr - 0x01000000:
        _log("found nonstandard ramdisk_addr\n")
    if not base == second_addr - 0x00F00000:
        _log("found nonstandard second_addr\n")
    if not base == tags_addr - 0x00000100:
        _log("found nonstandard tags_addr\n")
    if dt_size:
        _log("found device_tree_image\n")
    cmdline = cmdline[: cmdline.find(b"\x00")]

    _log("base: 0x%x\n" % base)
    _log("ramdisk_addr: 0x%x\n" % ramdisk_addr)
    _log("second_addr: 0x%x\n" % second_addr)
    _log("tags_addr: 0x%x\n" % tags_addr)
    _log("page_size: %d\n" % page_size)
    ...
    def gzname(x):
        return x == struct.pack("3B", 0x1F, 0x8B, 0x08) and ".gz" or ""
...
def write_cpio(cpiolist, output):
    ...
    def padding(x, y):
        return struct.pack("%ds" % ((~x + 1) & (y - 1)), b"")
    ...
    def write_cpio_header(output, ino, name, mode=0, nlink=1, filesize=0):
        ...
        def latin(x):
            return x.encode("latin")

        output.write(latin("070701"))
        output.write(
            latin("%08x" % ino)
        )  # Android自300000递增 # ino normally only for hardlink
    ...
    def cpio_tailer(output, ino):
        name = "TRAILER!!!"
        write_cpio_header(
            output, ino, name, 0o644
        )  # 8进制权限644, 的确应该为0? 为调用fix_stat引起的bug.
    ...
    functions_ = {
        ...
        "file": cpio_mkfile,
        "slink": cpio_mkslink,
        "nod": lambda *x: _log("No Supported Yet"),
        "pipe": lambda *x: _log("No Supported Yet"),
        "sock": lambda *x: _log("No Supported Yet"),
    }
    next_inode = 300000
    while True:
...
def repack_bootimg(
    _base=None, _cmdline=None, _page_size=None, _padding_size=None, cpiolist=None
):
    ...
    _log("arguments: [base] [cmdline] [page_size] [padding_size]\n")
    _log("kernel: kernel\n")
    _log("ramdisk: %s\n" % ramdisk)
    _log("second: %s\n" % second)
    _log("dt_image: %s\n" % dt_image)
...
def unpack_bootimg(bootimg=None, ramdisk=None, directory=None):
    ...
    _log("arguments: [bootimg file]\n")
    _log("bootimg file: %s\n" % bootimg)
    _log("output: kernel[.gz] ramdisk[.gz] second[.gz]\n")
...
def check_mtk_head(imgfile, outinfofile):
    ...
    if tag == 0x58881688:
        _log("Found mtk magic, skip header.\n")
        data = imgfile.read(0x4)
    ...
    if mode == "mtk":
        ...
        for line in imginfofile.readlines():
            ...
            if lines[0].strip() == "mtk_header_name":
                ...
                break
        data = struct.pack(
            "<II32s472s", magic, size, name.encode(), b"".ljust(472, b"\xff")
        )
        outfile.write(data)
...
def unpack_ramdisk(ramdisk=None, directory=None):
    ...
    _log("arguments: [ramdisk file] [directory]\n")
    _log("ramdisk file: %s\n" % ramdisk)
    _log("directory: %s\n" % directory)
    _log("output: cpiolist.txt\n")

    if os.path.lexists(directory):
        raise FileExistsError("Please remove existing directory: %s" % directory)
...
def repack_ramdisk(cpiolist=None):
    ...
    _log("arguments: [cpiolist file]\n")
    _log("cpiolist file: %s\n" % cpiolist)
    _log("output: ramdisk.cpio.gz\n")
