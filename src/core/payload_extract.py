...
class OrderedFileWriter(object):
    def __init__(self, file: BufferedWriter, max_workers: int):
        self.file = file
...
def init_payload_info(reader: IO[bytes]) -> update_metadata_pb2.DeltaArchiveManifest:
    ...
    if hdr.version != 2:
        emit(f"Warning: payload version is {hdr.version} != 2 may be unsupported")
...
def _extract_operation_to_file(
    ...
    if operation_type == update_metadata_pb2.InstallOperation.REPLACE:
        ...
        for extent in operation.dst_extents:
            out_seek = extent.start_block * block_size
        ...
        if operation_type == update_metadata_pb2.InstallOperation.REPLACE_BZ:
            decompressed_data = bz2.decompress(data)
        elif operation_type == update_metadata_pb2.InstallOperation.REPLACE_XZ:
...
def _extract_partition_from_payload(
    ...
    with open(out_path, "wb") as out_file:
        with OrderedFileWriter(out_file, executor._max_workers) as writer:
            ...
            emit(
                f"Extract partition: {partition.partition_name:<16} size: {total_size:<10} ... Done!"
            )
...
def extract_partitions_from_payload(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for p in all_parts:
            ...
            # emit(f"Extracting output size: {data_size}")
            ...
            emit(f"Extracting {p.partition_name} ...")
...
if __name__ == "__main__":
    ...
    emit("Extracting payload ...")
    now = time.time()

    selected_partitions = (
        args.extract_partitions.split(",") if args.extract_partitions else []
    )
    if args.type == "zip":
            ...
            with zipfile.ZipFile(input_stream, "r") as archive:
                for file_info in archive.filelist:
                    if file_info.filename.endswith("payload.bin"):
                        with archive.open(file_info, "r") as payload_stream:
        ...
        with open(args.input, "rb") as payload_stream:
            extract_partitions_from_payload(
                payload_stream, selected_partitions, args.out, args.workers
            )
    elif args.type == "url":
        with UrlFileReader(args.input) as remote_stream:
            with zipfile.ZipFile(remote_stream, "r") as archive:
                for file_info in archive.filelist:
                    if file_info.filename.endswith("payload.bin"):
                        with archive.open(file_info, "r") as payload_stream:
                            extract_partitions_from_payload(
        ...
        raise ValueError(f"Unsupported input type: {args.type}")
    tooks = time.time() - now

    emit("Done! tooks: %.2f" % tooks)
