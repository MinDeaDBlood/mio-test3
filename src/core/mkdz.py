...
class MKDZChunk(dz.DZChunk):
    ...
    def list(self, index):
        ...
        """
        emit("{:2d} : {:s}".format(index, self.chunkName))
    ...
    def write(self, file, name):
        ...
        length = input.seek(0, io.SEEK_END)
        input.seek(0, io.SEEK_SET)

        emit("[+] Writing {:s} to {:s} ({:d} bytes)".format(self.name, name, length))
    ...
    def __init__(self, name, blockShift):
        ...
        if (dz_item["targetSize"] >> blockShift) > dz_item["trimCount"]:
            emit(
                "[!] target size is more than number of blocks to wipe?!  Inconceivable!",
                file=sys.stderr,
            )
            raise CoreOperationError(
                "DZ chunk target size exceeds wipe block count", exit_code=1
            )
...
class MKDZFile(dz.DZFile):
    ...
    def loadParams(self):
        ...
        while len(line) > 0:
            ...
            if len(parts[1]) == 0:
                emit(
                    "[!] Bad line in {:s} parameter file".format(".dz.params"),
                    file=sys.stderr,
                )
            var = parts[0].rstrip()
            try:
        ...
        for k in self._dz_format_dict.keys():
            ...
            if k not in params:
                emit(
                    '[!] Parameter value "{:s}" is missing, unable to continue'.format(
                        k
                    ),
                    file=sys.stderr,
                )
                raise CoreOperationError(
                    f"Missing required DZ parameter: {k}", exit_code=1
                )
            elif hasattr(params[k], "encode"):
                params[k] = params[k].encode("utf8")

        if params["formatMajor"] != 2:
            emit("[!] File format version not 2, cannot continue", file=sys.stderr)
            raise CoreOperationError("Unsupported DZ file format version", exit_code=1)

        if params["formatMinor"] != 1:
            emit(
                "[!] Warning: File appears to be a v{:d}.{:d} format file, compatibility uncertain!".format(
                    params["format_major"], params["format_minor"]
                ),
                file=sys.stderr,
            )
    ...
    def loadChunks(self):
        ...
        self.chunks.sort(
            key=lambda c: (
                c.getStart()
                + (c.getDev() << 48)
                + (1 << 56 if c.chunkName[-4:] == ".img" else 0)
            )
        )
    ...
    def checkChunks(self):
        ...
        for chunk in self.chunks:
            ...
            if chunk.getStart() < last:
                emit(
                    "[!] chunk {:s} overlaps!".format(chunk.chunkName), file=sys.stderr
                )
                raise CoreOperationError(
                    f"DZ chunk overlaps: {chunk.chunkName}", exit_code=1
                )
            last = chunk.getEnd()
    ...
    def listChunks(self):
        ...
        emit("[+] Found {:d} chunks:".format(len(self.chunks)))
        emit()
    ...
    def writeFile(self, file, name):
        ...
        emit("[+] Writing {:d} chunks to {:s}:".format(len(self.chunks), name))
        emit()
...
class MKDZFileTools:
    ...
    def parseArgs():
        ...
        parser = argparse.ArgumentParser(
            description="LG Compressed DZ File Creator by Elliott Mitchell"
        )
        parser.add_argument(
            "-f",
            "--file",
            help="DZ File to create",
            action="store",
            required=True,
            dest="dzfile",
        )
        ...
        group.add_argument(
            "-m",
            "--make",
            help="make DZ file from chunks in directory",
            action="store_true",
            dest="createFile",
        )
        parser.add_argument(
            "-d", "--dir", help="input location", action="store", dest="indir"
        )
