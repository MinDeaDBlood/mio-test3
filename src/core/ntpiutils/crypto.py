    ...
    if key_hex and iv_hex:
        try:
            ...
            raise CoreOperationError(
                f"Failed to decode AES key for region {region_type}: {exc}",
                exit_code=-1,
            ) from exc
    raise CoreOperationError(
        f"AES key or IV was not found for region {region_type}",
        exit_code=-1,
    )
...
def aes_cbc_decrypt(encrypted_data, key=None, iv=None):
    ...
    try:
        ...
        raise CoreOperationError(f"AES decryption failed: {exc}", exit_code=-1) from exc
