...
def init_worker(region6_data_blob, keymap_data_blob, pbar_lock, completed_counter):
    ...
    if G_REGION6_DATA is None or G_KEYMAP_DATA is None:
        raise CoreOperationError("Worker global data is not initialized", exit_code=1)
...
def decompress_lzma2_data(decrypted_data):
    ...
    if len(decrypted_data) < ctypes.sizeof(NTDecompressHeader):
        raise CoreOperationError(
            "Data is too small for NTDecompressHeader",
            exit_code=-1,
        )
    ...
    if not decrypted_data.startswith(b"NTENCODE"):
        raise CoreOperationError(
            "Invalid NTDecompressHeader magic",
            exit_code=-1,
        )

    # Parse decompression header
    _nt_decompress_header = NTDecompressHeader.from_buffer_copy(
        decrypted_data[: ctypes.sizeof(NTDecompressHeader)]
    )
    ...
    if data_offset >= len(decrypted_data):
        raise CoreOperationError(
            "Data offset exceeds the LZMA block range",
            exit_code=-1,
        )
    ...
    try:
        ...
        decompressed_data = lzma.decompress(
            compressed_data, format=lzma.FORMAT_RAW, filters=[{"id": lzma.FILTER_LZMA2}]
        )
        return decompressed_data
    except lzma.LZMAError as e:
        emit(f"LZMA2 decompression failed: {e}")
        raise
    except Exception as e:
        emit(f"Unknown error during decompression: {e}")
        raise
...
def split_file_into_segments(fileinfo, num_segments=4):
    ...
    while current_offset < offset_end:
        ...
        nt_header = NTEncodeHeader.from_buffer_copy(
            G_REGION6_DATA[
                current_offset : current_offset + ctypes.sizeof(NTEncodeHeader)
            ]
        )
    ...
    for i in range(total_blocks):
        current_segment_size = (
            block_boundaries[i][2] - block_boundaries[segment_start_idx][2]
        )
        ...
        if should_end_segment or i == total_blocks - 1:
            ...
            if i == total_blocks - 1:
                ...
                end_offset = (
                    block_boundaries[i + 1][0] if i + 1 < total_blocks else offset_end
                )
...
def process_segment(fileinfo, segment_info, segment_id):
    ...
    while current_offset < end_offset:
        ...
        next_offset, decrypted_data = decrypt_nt_encode_data(
            G_REGION6_DATA, current_offset, key
        )
...
def process_large_file_parallel(fileinfo, files_output_dir, position):
    ...
    try:
        ...
        with G_PBAR_LOCK:
            emit(f"\n{'=' * 80}")
            emit(f"Large File Processing: {filename}")
            emit(f"   File Size: {expected_size / (1024**2):.2f} MB")
            emit(f"   CPU Cores: {os.cpu_count()}, Optimal Segments: {num_segments}")
            emit("   Analyzing block structure...")
        ...
        with G_PBAR_LOCK:
            emit(
                f"   Split into {len(segments)} segments, ~{expected_size / (1024**2) / len(segments):.2f} MB each"
            )
            for i, seg in enumerate(segments):
                emit(f"     Segment {i + 1}: {seg['num_blocks']} blocks")
            emit(f"   Starting {num_segments}-thread parallel processing...")
        ...
        with ThreadPoolExecutor(max_workers=num_segments) as executor:
            ...
            for seg_id, segment_info in enumerate(segments):
                ...
                future = executor.submit(
                    process_segment, fileinfo, segment_info, seg_id
                )
            ...
            for seg_id, future in enumerate(futures):
                with G_PBAR_LOCK:
                    emit(f"   Waiting for segment {seg_id + 1}...")
                seg_data = future.result()
                segment_results.append(seg_data)
                with G_PBAR_LOCK:
                    emit(
                        f"   Segment {seg_id + 1} completed ({len(seg_data) / (1024**2):.2f} MB)"
                    )

        # Merge all segments into final file
        with G_PBAR_LOCK:
            emit("   Merging all segments...")
        ...
        if actual_size != expected_size:
            with G_PBAR_LOCK:
                emit(
                    f"   Size mismatch: Expected {expected_size / (1024**2):.2f} MB, "
        ...
        if actual_hash != expected_hash:
            with G_PBAR_LOCK:
                emit("   Hash verification failed!")
            return filename, False, "Hash mismatch"

        # Write final data to output file
        with G_PBAR_LOCK:
            emit("   Writing to disk...")
        ...
        with G_PBAR_LOCK:
            emit(f"   Completed! Time: {process_time:.2f}s, Speed: {speed:.2f} MB/s")
            emit(f"{'=' * 80}\n")
...
def process_file_task(fileinfo, files_output_dir, position):
    ...
    try:
        ...
        while True:
            ...
            process_offset, decrypted_data = decrypt_nt_encode_data(
                G_REGION6_DATA, process_offset, key
            )
        ...
        if actual_size != expected_size:
            with G_PBAR_LOCK:
                emit(
                    f"Warning: {filename} size mismatch. Expected {expected_size}, Got {actual_size}"
                )
...
def stage2_extract_files(
    temp_dir, final_output_dir, all_files=True, process_count=None
):
    """
    ...
    for path in [fileindex_path, keymap_path, region6_path]:
        if not os.path.exists(path):
            raise CoreOperationError(
                f"Required stage 2 file was not found: {path}",
                exit_code=-1,
            )
    ...
    try:
        ...
        emit(f"Failed to create output directory: {e}")
        files_output_dir = final_output_dir
    ...
    if not files_info:
        raise CoreOperationError(
            "No file information was found in FileIndex.xml",
            exit_code=-1,
        )

    # Test mode: process only first file
    if not all_files:
        emit("--- Running in test mode: processing only the first file. ---")
    ...
    if large_files:
        emit(
            f"Detected {len(large_files)} large file(s) (>=500MB), will use multi-threaded acceleration"
        )
        emit(
            f"Strategy: Process {len(small_files)} small files first, then large files"
        )

    # Load Region6 and KeyMap into memory for fast access
    emit("Loading data into memory...")
    try:
        ...
        raise CoreOperationError(
            "Not enough memory to load NTPI extraction data",
            exit_code=1,
        ) from exc
    except OSError as exc:
        raise CoreOperationError(
            f"Cannot load NTPI extraction data: {exc}",
            exit_code=1,
        ) from exc
    ...
    with multiprocessing.Pool(
        ...
        try:
            ...
            pool.terminate()
            pool.join()
            raise CoreOperationError("NTPI extraction was interrupted", exit_code=1)
    ...
    for name, success, message in pool_results:
        if success:
            ...
            fail_count += 1
            emit(f"Failed to extract {name}: {message}")

    # Print stage 2 summary
    stage2_elapsed = time.time() - stage2_start
    emit(f"\nStage 2 Complete. Success: {success_count}, Failed: {fail_count}")
    emit(f"Stage 2 Time: {stage2_elapsed:.2f}s ({stage2_elapsed / 60:.2f} min)")
    if success_count > 0:
        avg_time = stage2_elapsed / success_count
        emit(f"Average time per file: {avg_time:.2f}s")
