...
def extract_region_data(file_data, region_header, offset, output_dir, keys_dict=None):
    ...
    if offset + region_header.region_size > len(file_data):
        raise CoreOperationError(
            f"Region data is out of bounds for {region_name}",
            exit_code=-1,
        )
    ...
    if len(decrypted_data) < ctypes.sizeof(RegionBlockHeader):
        raise CoreOperationError(
            f"Decrypted data for {region_name} is too small for a RegionBlockHeader",
            exit_code=-1,
        )
    ...
    if data_offset + block_header.real_size > len(decrypted_data):
        raise CoreOperationError(
            f"Real data size for {region_name} exceeds the decrypted data buffer",
            exit_code=-1,
        )
...
def parse_ntpi_file(file_path, output_dir):
    ...
    """
    emit("=== Stage 1: Parsing NTPI File... ===")
    stage1_start = time.time()
    
    # Validate file existence
    if not os.path.exists(file_path):
        emit(f"Error: Input file not found: {file_path}")
        return False
    ...
    if len(file_data) < ctypes.sizeof(NTPIHeader):
        emit("Error: File is too small to be a valid NTPI file.")
        return False
    
    # Parse NTPI header
    ntpi_header = NTPIHeader.from_buffer_copy(file_data[:ctypes.sizeof(NTPIHeader)])
    if not ntpi_header.is_valid():
        emit("Error: Invalid NTPI file magic.")
        return False
    
    emit(f"NTPI header parsed successfully. Version: {ntpi_header.version_major}.{ntpi_header.version_minor}.{ntpi_header.version_patch}")
    ...
    if keys_dict is None:
        emit(f"Error: Unsupported firmware version {ntpi_header.version_major}.{ntpi_header.version_minor}.{ntpi_header.version_patch}")
        emit("This version is not currently supported. Please add AES keys to utils/structures.py")
        emit("Supported versions:")
        from .structures import VERSION_KEY_MAP
        for ver in VERSION_KEY_MAP.keys():
            emit(f"  - Version {ver[0]}.{ver[1]}.{ver[2]}")
        return False
    
    emit(f"Using AES keys for version {ntpi_header.version_major}.{ntpi_header.version_minor}.{ntpi_header.version_patch}")
    ...
    emit(f"Stage 1 completed. Parsed {region_count} regions.")
    emit(f"Stage 1 Time: {stage1_elapsed:.2f}s")
    return True
...
def parse_fileindex_xml(xml_path):
    ...
    if not xml_path.exists():
        emit(f"Error: FileIndex.xml not found at {xml_path}")
        return []
    try:
        ...
        emit(f"Parsed {len(files_info)} file entries from FileIndex.xml.")
        return files_info
    except (ET.ParseError, OSError, TypeError, ValueError) as exc:
        raise CoreOperationError(
            f"Cannot parse FileIndex.xml: {exc}",
            exit_code=-1,
        ) from exc
