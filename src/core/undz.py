...
class DZFileTools:
    ...
    def getPartitions(self):
        ...
        while True:
            ...
            if dz_sub['header'] != self.dz_sub_header:
                raise CoreOperationError('Bad DZ sub header', exit_code=0)
    ...
    def openFile(self, dzfile):
        ...
        if verify_header != self.dz_header:
            expected = ' '.join(hex(ord(str(n))) for n in self.dz_header)
            received = ' '.join(hex(ord(str(n))) for n in verify_header)
            raise CoreOperationError(
                f'Unsupported DZ file format. Expected {expected}, received {received}',
                exit_code=0,
            )
    ...
    def cmdListPartitions(self):
        emit("[+] DZ Partition List\n=========================================")
        for part in enumerate(self.partList):
            emit("%2d : %s (%d bytes)" % (part[0], part[1][0].decode(), part[1][1]))

    def cmdExtractSingle(self, partID):
        emit("[+] Extracting single partition!\n")
        emit("[+] Extracting {self.partList[partID][0].decode()} to " + os.path.join(self.outdir,
                                                                                        self.partList[partID][0].decode(())))
        self.extractPartition(partID)

    def cmdExtractAll(self):
        emit("[+] Extracting all partitions!\n")
        for part in enumerate(self.partList):
            emit(f"[+] Extracting {part[1][0].decode()} to {os.path.join(self.outdir, part[1][0].decode())}")
