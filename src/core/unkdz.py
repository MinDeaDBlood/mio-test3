...
class KDZFileTools(kdz.KDZFile):
    ...
    def __init__(self, kdzfile: str, outdir: str, extract_id: int = None, list_only: bool = False,
        ...
        if list_only:
            ...
            if 0 <= extract_id < len(self.partList):
                ...
                emit("[!] Segment {:d} is out of range!".format(extract_id))
        ...
        for key in self._dz_collapsibles:
            if type(kdz_item[key]) is str or type(kdz_item[key]) is bytes:
                ...
                if b'\x00' in kdz_item[key]:
                    emit("[!] Error: extraneous data found IN " + key)
                    return
            elif type(kdz_item[key]) is int:
                if kdz_item[key] != 0:
                    emit('[!] Error: field "' + key + '" is non-zero (' + b2a_hex(kdz_item[key]) + ')',
                          )
                    return
            else:
    ...
    def getPartitions(self):
        ...
        if len(buf.lstrip(b'\x00')) > 0:
            emit("[!] Warning: Data between headers and payload! (offsets {:d} to {:d})".format(self.headerEnd,
                                                                                                 self.dataStart))
            self.hasExtra = True
    ...
    def saveExtra(self):
        ...
        emit("[+] Extracting extra data to " + filename)
    ...
    def openFile(self, kdzfile):
        ...
        if verify_header not in self.kdz_header:
            emit("[!] Error: Unsupported KDZ file format.")
            emit('[ ] Received header "{:s}".'.format(" ".join(b2a_hex(n) for n in verify_header)))
            return

        self.header_type = self.kdz_header[verify_header]

    def cmdExtractSingle(self, partID):
        emit(f"[+] Extracting single partition from v{self.header_type:d} file!\n")
        emit("[+] Extracting " + str(self.partList[partID][0]) + " to " + os.path.join(self.outdir,
                                                                                        self.partList[partID][0].decode(
                                                                                            "utf8")))
        self.extractPartition(partID)

    def cmdExtractAll(self):
        emit(f"[+] Extracting all partitions from v{self.header_type:d} file!\n")
        for part in enumerate(self.partList):
            emit("[+] Extracting " + part[1][0].decode("utf8") + " to " + os.path.join(self.outdir,
    ...
    def cmdListPartitions(self):
        ...
        for part in enumerate(self.partList):
            emit(f"{part[0]:2d} : {part[1][0].decode('utf8'):s} ({part[1][1]:d} bytes)")
