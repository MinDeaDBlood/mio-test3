...
class Sdat2img:
    def __init__(self, transfer_list_file, new_data_file, output_image_file):
        emit('sdat2img binary - version: 1.3\n')
        ...
        emit("Android {} detected!\n".format(versions.get(version, f'Unknown version {version}!\n')))
        # Don't clobber existing files to avoid accidental data loss
        try:
            output_img = open(self.output_image_file, 'wb')
        except IOError as e:
            if e.errno == 17:
                emit(f'Error: the output file "{e.filename}" already exists')
                emit('Remove it, rename it, or choose a different file name.')
                return
            else:
                emit(e)
                return
        ...
        for cmd, block_list in self.list_file:
            ...
            for begin, block_all in block_list:
                ...
                emit(f'Copying {block_count} blocks into position {begin}...')
        ...
        output_img.close()
        new_data_file.close()
        emit(f'Done! Output image: {os.path.realpath(output_img.name)}')
    ...
    def rangeset(src):
        ...
        if len(num_set) != num_set[0] + 1:
            emit(f'Error on parsing following data to rangeset:\n{src}')
            return
        ...
        with open(self.transfer_list_file, 'r', encoding='utf-8') as trans_list:
            ...
            for line in trans_list:
                ...
                if cmd == 'new':
                    ...
                    if cmd in ['erase', 'new', 'zero']:
                        emit(f'Skipping command {cmd}...')
                        continue
                    # Skip lines starting with numbers, they are not commands anyway
                    if not cmd[0].isdigit():
                        emit(f'Command "{cmd}" is not valid.')
                        return
