...
SPEC = PackOutputSpec(key='br', description='pack brotli-compressed dat output')


def apply_output(request: PackOutputRequest, *, output=None):
    image_path = f"{request.work_output}{request.partition_name}.img"
    img2simg(image_path)
    return datbr(request.work_output, request.partition_name, request.brotli_level, int(request.dat_version), output=output)
